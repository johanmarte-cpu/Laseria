...
export function ConsentForm({ defaultName, consent }: { defaultName: string; consent: Consent }) {
  ...
  async function handleSubmit() {
    ...
    if (!res.ok) {
      setError(body.error ?? "No pudimos registrar tu consentimiento.");
      return;
    }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <FormField label="Nombre completo (firma)" htmlFor="consent-name">
          <Input id="consent-name" value={fullName} onChange={(e) => setFullName(e.target.value)} />
        </FormField>
        <FormField label="Cédula" htmlFor="consent-cedula" optional>
          <Input
            id="consent-cedula"
      ...
      <label className="flex items-start gap-2.5 text-sm text-ink-soft">
        ...
        />
        He leído y comprendido el consentimiento informado descrito arriba, y autorizo a que se me realice el
      ...
      <label className="flex items-start gap-2.5 text-sm text-ink-soft">
        ...
        />
        (Opcional) Autorizo el uso de fotografías de antes/después de mi tratamiento con fines de marketing de{" "}
        {"Lasería"}.
      </label>

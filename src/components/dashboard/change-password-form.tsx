...
export function ChangePasswordForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
      ...
      <FormField label="Contraseña actual" htmlFor="cp-current" error={errors.currentPassword?.message}>
        <Input id="cp-current" type="password" {...register("currentPassword")} />
      </FormField>

      <div className="grid gap-5 sm:grid-cols-2">
        <FormField label="Nueva contraseña" htmlFor="cp-new" error={errors.newPassword?.message}>
          <Input id="cp-new" type="password" {...register("newPassword")} />
        </FormField>
        <FormField label="Confirmar nueva contraseña" htmlFor="cp-confirm" error={errors.confirmPassword?.message}>
